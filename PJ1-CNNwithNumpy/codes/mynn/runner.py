...
class RunnerM():
    ...
    """
    def __init__(self, model, optimizer, metric, loss_fn, batch_size=32, scheduler=None,reg=False):
        self.model = model
    ...
    def train(self, train_set, dev_set, **kwargs):
        ...
        for epoch in range(num_epochs):
            ...
            for iteration in range(int(X.shape[0] / self.batch_size) + 1):
                ...
                if self.reg:
                    reg_layer = L2Regularization(self.model)
                    reg_loss = reg_layer.forward()
                    # print("calculate the reg loss: {}".format(reg_loss))
                    trn_loss = trn_loss + reg_loss
